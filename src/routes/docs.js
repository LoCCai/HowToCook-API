...
router.get('/', (req, res) => {
  const rows = [
    ...
    ['GET', '/api/menu', '自动配一餐（六槽位自由组合：荤/素/汤默认各 1，早餐/饮料/甜品默认 0）。参数：seed、各槽位数、max_difficulty、exclude_tags、image_mode'],
    ['GET', '/api/plan/week', '一周膳食计划（默认每日一荤一素一汤；可选早餐/饮料/甜品槽、按天槽数如 meat=1,2,1、周内不重样、seed 可复现）。参数：days、meat/vegetable/soup/breakfast/drink/dessert、max_difficulty、exclude_tags、seed、with_shopping_list=1、servings、image_mode'],
    ...
    ['GET', '/api/recipes/:id/notes', '附加内容与反馈声明（text=Markdown，html=渲染后，参考资料链接可点击）'],
...
function openApiDocument() {
  ...
  return {
    ...
    paths: {
      ...
      '/api/recipes/{id}/notes': { get: { summary: '附加内容（text + html；参考资料等行内链接在 html 中渲染）', parameters: [idParam(), imageModeParam()], responses: okRef() } },
      ...
      '/api/plan/week': {
        get: {
          summary: '一周膳食计划：默认每日一荤一素一汤；六槽位可配（含早餐/饮料/甜品）、支持按天槽数（如 meat=1,2,1）、周内不重样；seed 可复现整周',
          parameters: [
            q('seed', '随机种子：相同 seed 返回相同整周'),
            q('days', '计划天数，默认 7，最大 14', { type: 'integer', default: 7 }),
            q('meat', '每日荤菜数；可按天指定如 1,2,1（默认 1，每槽上限 3）'),
            q('vegetable', '每日素菜数；同上'),
            q('soup', '每日汤数；同上'),
            q('breakfast', '每日早餐数（默认 0，来自早餐分类）'),
            q('drink', '每日饮料数（默认 0）'),
            q('dessert', '每日甜品数（默认 0）'),
            q('max_difficulty', '难度上限 1-5', { type: 'integer', minimum: 1, maximum: 5 }),
            q('exclude_tags', '排除含这些标签的菜（忌口/过敏原）'),
            q('with_shopping_list', '传 1 时响应附整周合并购物清单'),
            q('servings', '购物清单份数缩放（基准 2 人份）', { type: 'integer' }),
            imageModeParam(),
          ],
          responses: okRef(),
        },
